...
class _DoctorHomeState extends State<DoctorHome> {
  ...
  Future<void> fetchBookings() async {
    try {
      final dio = Dio();
      final doctorId = Storage.get('id');
      final response = await dio
          .get('https://192.168.1.219:3000/appointments/today/$doctorId');

      if (response.statusCode == 200) {
        setState(() {
          // Assuming the API returns lists for both online and offline bookings
          offlineBooking = List<Map>.from(response.data['offline']);
          onlineBooking = List<Map>.from(response.data['online']);
          isLoading = false;
        });
      } else {
        // Handle error
